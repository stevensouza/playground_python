...
class GoogleSheet:
    ...
    def get_data(self, google_sheet_id, sheet_a1_notation):
        """
            Returns a list of lists where each row of the spreadsheet is a list.  The first row/list could contain
            the header if the spreadsheet is set up that way. Example: [["name","age"],["joe",82],["bo",33]]
        """
    ...
    def put_data(self, google_sheet_id, sheet_a1_notation, rows, value_input_option="USER_ENTERED", overwrite_or_append="OVERWRITE"):
        ...
        :param rows: rows as a list of lists [['joe','smith'],['joe','jones']]. Note a header can optionally
